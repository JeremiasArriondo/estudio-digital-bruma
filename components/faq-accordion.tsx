  ...
  return (
    <div className="space-y-4">
      {faqs.map((faq, index) => (
        ...
        >
          <button onClick={() => toggleFaq(index)} className="w-full p-4 flex items-center justify-between text-left">
            <span className="font-semibold text-purple-700 dark:text-purple-400">{faq.question}</span>
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${
                activeIndex === index
                  ? "bg-gradient-to-r from-purple-600 to-fuchsia-600 dark:from-purple-500 dark:to-fuchsia-500 scale-110"
                  : "bg-purple-600 dark:bg-purple-500 hover:scale-110"
              }`}
            >
              <motion.span
                animate={{ rotate: activeIndex === index ? 45 : 0, opacity: activeIndex === index ? 0 : 1 }}
              ...
              <motion.span
                animate={{ rotate: activeIndex === index ? 0 : 45, opacity: activeIndex === index ? 1 : 0 }}
                transition={{ duration: 0.3 }}
                className="text-white font-bold text-xl absolute"
              >
                −
            ...
            {activeIndex === index && (
              ...
              >
                <div className="p-4 pt-0 border-t-2 border-purple-200 dark:border-slate-700">
                  <p className="text-purple-900 dark:text-purple-300 leading-relaxed">{faq.answer}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
